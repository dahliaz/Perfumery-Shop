...
protected:
  char size;
  int peach, rose, vanilla, musk,
      sandalwood; // variables needed to multiply with prices of scents to get
                  // total price
...
protected:
  ...
  void scentMenu() { // for users to input a number to enable switch case
  ...
  void getScents(int i) { // set teh number of scent drops 
  ...
  char uniqueChoice() { // do they want multiple bottles of the same or
                        // different scents?
    cout << "---------------------------------------------------------\n";
	while (1) {
...
public:
  ...
  int scentChoice() { // get 3 scents they want, the switch case will help us to
